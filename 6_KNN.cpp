...
int main() {
    ...
    cout << endl << "The time taken is: " << (float)time_req / CLOCKS_PER_SEC << " seconds" << endl;

    // Calculate bytes occupied
    int bytes = sizeof(coords) * 7 + sizeof(action) + sizeof(comedy) + sizeof(farthest_pointA) + sizeof(farthest_pointC) + sizeof(nearest_pointA) + sizeof(nearest_pointC) + sizeof(centroidA) + sizeof(centroidC) + sizeof(c_A) + sizeof(c_C);
    cout << "The bytes occupied is: " << bytes << " bytes" << endl;
